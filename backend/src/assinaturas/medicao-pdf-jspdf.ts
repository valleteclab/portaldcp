  ...
  if (dados.itens && dados.itens.length > 0) {
    ...
    const body: any[][] = dados.itens.map((item: any) => {
      ...
      const cNo =
        item.valor_no_periodo !== undefined && item.valor_no_periodo !== null
          ? Math.round(
              truncarMoedaReais2Casas(Number(item.valor_no_periodo)) * 100,
            )
          : produtoQuantidadeValorUnitarioCentavos(
              item.quantidade_no_periodo,
              vu,
      ...
      // Item recorrente (unidades/mês × meses): cada coluna fiscal com meses em cima e unidades embaixo
      const rec = item.recorrente?.texto_colunas;
      return [
        { content: item.numero, styles: { halign: 'center' as const, fontSize: 6 } },
        { content: item.descricao, styles: { fontSize: 6 } },
        { content: rec?.no_periodo ?? fiscalNo, styles: { halign: 'center' as const, fontSize: 6 } },
        { content: rec?.ate_periodo ?? fiscalAte, styles: { halign: 'center' as const, fontSize: 6 } },
        { content: rec?.a_executar ?? fiscalExec, styles: { halign: 'center' as const, fontSize: 6 } },
  ...
  if (dados.etapas && dados.etapas.length > 0) {
    ...
    autoTable(doc, {
      ...
      head: [
        ...
        [
          { content: 'ANT.', styles: { halign: 'center' as const, fontStyle: 'bold' as const, fontSize: 5.4, fillColor: etapaHeadFisicaSub, textColor: [255, 255, 255] as [number, number, number] } },
          { content: 'PER.', styles: { halign: 'center' as const, fontStyle: 'bold' as const, fontSize: 5.4, fillColor: etapaHeadFisicaSub, textColor: [255, 255, 255] as [number, number, number] } },
          ...
          { content: 'ANT.', styles: { halign: 'right' as const, fontStyle: 'bold' as const, fontSize: 5.4, fillColor: etapaHeadFinanceiraSub, textColor: [255, 255, 255] as [number, number, number] } },
          { content: 'PER.', styles: { halign: 'right' as const, fontStyle: 'bold' as const, fontSize: 5.4, fillColor: etapaHeadFinanceiraSub, textColor: [255, 255, 255] as [number, number, number] } },
