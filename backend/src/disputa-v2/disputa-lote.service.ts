...
export class DisputaLoteService {
  ...
  async iniciar(
    ...
    await this.transicoes.executar(licitacaoId, AtoLicitacao.INICIAR_DISPUTA, {
      ...
      registro: { origem: 'disputa-v2/lote', sessao_id: sessao.id },
    });
  ...
  async listarMeusLances(sessaoId: string, loteId: string, fornecedorId: string, prazoSegundos: number): Promise<LancePainelCancelamentoV3[]> {
    const m = this.dataSource.manager;
