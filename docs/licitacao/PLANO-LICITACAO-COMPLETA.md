...
- **Incremento 2.7 (25/07/2026 — IMPLEMENTADO): FASE DE LANCES da dispensa** (modelo IN SEGES 67/2021, sem tocar no motor disputa-v2). Entidade própria `dispensa_lances` + campos `dispensa_lances_inicio/fim` na licitação. Órgão abre a janela pelo cockpit (opcional, duração em minutos, default 6h) após o fim do acolhimento; fornecedor entra na **sala de lances** (`/fornecedor/licitacoes/[id]/lances`, botão pulsante no detalhe) e reduz o PRÓPRIO valor por item (validação: menor que o próprio último valor; só quem tem proposta válida). Menor valor por item é **público e anônimo** (painel `GET :id/dispensa/lances/painel`, polling 5s, countdown). Sigilo público estendido durante a janela. `julgarDispensa` considera `min(proposta, lances do fornecedor)` por item e fica bloqueado enquanto a janela está aberta; cockpit mostra painel ao vivo e trava o botão Julgar. Lances são opcionais — dispensa sem lances julga direto das propostas. **Tempo real (mesma entrega):** gateway WebSocket `/dispensa` (padrão do /disputa-v2) — push instantâneo de novo menor valor (`painel_atualizado`), chat (`chat`) e abertura de janela (`janela`); polling de 10s vira retaguarda; badge AO VIVO/reconectando; **countdown sincronizado pelo relógio do SERVIDOR** (offset calculado de `server_time` — imune a relógio errado no PC do fornecedor). **Chat da dispensa** (`dispensa_mensagens` + GET público/POST): registrado nos autos, autoria do fornecedor ANÔNIMA enquanto a janela de lances está aberta (mascarada na leitura, preservada no registro), chat encerra na homologação; usado para avisos e para a NEGOCIAÇÃO pós-lances com o melhor classificado (modelo Compras.gov/IN 67). UI: chat na sala do fornecedor e no cockpit do órgão (painel/chat com atualização automática durante a janela).
- **Incremento 2.2 (25/07/2026 — IMPLEMENTADO): PNCP como efeito de transição (D5).** (a) `publicarEdital` de DISPENSA dispara automaticamente `pncpService.enviarCompraCompleta` (compra + itens; lógica movida do controller para o service); (b) `homologar` dispara `enviarResultadoHomologacao` (novo — ResultadoItemDto por item vencedor: NI/razão do fornecedor, quantidades/valores homologados, porte ME/EPP/DEMAIS; pulado quando `selecao_externa`, pois a plataforma de origem publica); (c) o **documento obrigatório da compra deixou de ser PDF placeholder**: para dispensa/inexigibilidade é gerado o **AVISO DE CONTRATAÇÃO DIRETA real** (`aviso-dispensa-pdf.ts` — identificação, objeto, itens, prazos, forma de participação); (d) ambos fire-and-forget (falha não bloqueia o ato; fica em `pncp_sync`); (e) cockpit mostra **status PNCP** (chips por sync com nº de controle/erro) e botões "Publicar/Reenviar aviso" (`POST /pncp/compras/:id/completo`) e "Enviar resultado" (`POST /pncp/compras/:id/resultados-homologacao` — rota nova); (f) `processo-completo` retorna `pncp[]`. Ambiente default = treina.pncp.gov.br (PNCP_API_URL configura produção).
- **Incremento 2.4 (25/07/2026 — IMPLEMENTADO): INSTRUÇÃO DO PROCESSO (Art. 72) — a "fase antes" da dispensa.** Pesquisa comparativa (Compras.gov/IN 67 + sistemas de mercado) confirmou: o rito de 5 fases estilo pregão era inadequado para contratação direta (exigia até "Designação de Pregoeiro"). Entregue: (a) checklist do Art. 72 para dispensa/inexigibilidade — obrigatórios **DFD + estimativa de despesa (PP) + autorização (AA)**; ETP/TR/riscos/parecer/dotação/justificativa como "se for o caso", marcáveis **"não se aplica" com justificativa registrada nos autos** (`GET /fase-interna/:id/instrucao`, `POST .../instrucao/:tipo/nao-se-aplica`, com desfazer; obrigatórios não dispensáveis); (b) **etapa única** — contratação direta não percorre as 5 fases; checklist completo → pronto para divulgar (`avancarFaseInterna` com desvio); documento "conta" com conteúdo/arquivo, sem fluxo formal de aprovação; (c) **gate de divulgação** — `publicar-edital` de dispensa/inexigibilidade recusa instrução incompleta listando pendências (padrão do card de pendências do Compras.gov); (d) cockpit: card "Instrução do processo (Art. 72)" (status por documento com fundamento legal + ações "não se aplica"/abrir) e **botão "Divulgar aviso da dispensa"** (antes não havia divulgação na UI!) — habilita com o mínimo verde, modal sugere 3 dias úteis (art. 75 §3º), conclui instrução + publica + dispara PNCP automático. Vantagem sobre o Compras.gov: itens/valores/PCA já vêm da demanda — a instrução nasce preenchida.
- **PNCP validado de ponta a ponta no treina (25/07/2026):** aviso publicado AUTOMATICAMENTE ao divulgar (compra 62203670000162-1-000002/2026) e **resultado por item confirmado na consulta pública (HTTP 200 com os vencedores)** após a cadeia de campos obrigatórios do resultado (percentualDesconto, tipoPessoaId/porte/país/situação, e indicadores aplicacaoMargemPreferencia/aplicacaoBeneficioMeEpp/aplicacaoCriterioDesempate — o PNCP valida um por vez).
- **Incremento 2.5 (25/07/2026 — IMPLEMENTADO): módulo `contratacao-direta` órfão APOSENTADO.** Removidos backend (`backend/src/contratacao-direta/` — module/controller/service/entities, sem nenhum outro import) e frontend (`/app/contratacao-direta`, que tinha link interno quebrado; botão no credenciamento removido). Tabelas `contratacoes_diretas`/`itens_contratacao_direta` estavam com 0 registros no ambiente de testes; as tabelas não são derrubadas (TypeORM não remove tabelas desconhecidas). Caminho único confirmado: dispensa/inexigibilidade = modalidade dentro de licitações.
- **Incremento 2.6 (25/07/2026 — IMPLEMENTADO): prorrogação automática da janela de lances (OPCIONAL).** Fundamentação verificada na fonte primária: a IN SEGES 67/2021 atualizada (art. 11, § único) encerra a janela SEM prorrogação — o mecanismo vem do modo de disputa aberto (art. 56, §1º, Lei 14.133) e não é vedado para dispensa em sistema próprio, desde que previsto nas regras da sessão. Implementação: órgão define ao abrir a janela (campo `prorrogacao_minutos`, 0 = encerramento seco padrão IN 67; sugestão 2 min); lance nos últimos N min empurra o fim para +N min, sucessivamente; novo fim é emitido via WebSocket (`janela`) + registrado no chat da sessão pelo autor "Sistema" (a regra é anunciada na abertura e cada prorrogação fica nos autos/ata); painel/polling também devolvem o fim atualizado. Campo novo `dispensa_lances_prorrogacao_min` na licitação.
- **Incremento 2.7-PNCP (25/07/2026 — IMPLEMENTADO): CONTRATOS no PNCP (art. 94).** A divulgação do contrato no PNCP é condição de EFICÁCIA (10 dias úteis na contratação direta, 20 na licitação). `enviarContratosHomologacao`: todos os contratos da licitação com `numeroControlePNCPCompra` (vínculo à compra), CNPJ do órgão da licitação, categoria derivada do tipo de contratação; já enviados são pulados; sync em `pncp_sync` tipo CONTRATO. Hook na homologação (fire-and-forget) + rota de reenvio + botão/chips no cockpit.
- **Incremento 2.8 (25/07/2026 — IMPLEMENTADO): CONTRATO FIM A FIM — termo gerado + assinatura eletrônica de todas as partes + PNCP.** (a) `contrato-pdf.ts`: TERMO DE CONTRATO gerado pelo sistema (preâmbulo com qualificação das partes, cláusulas essenciais do art. 92, tabela de itens do fornecedor, fundamentação dispensa/inexigibilidade, página final de assinaturas com posições fixas); (b) `POST /contratos/:id/solicitar-assinaturas`: gera o PDF e cria o documento no ASSINADOR já existente (signatários: responsável do órgão [assina por OTP no Portal de Assinaturas] + representante do fornecedor [link com token por e-mail]; extras opcionais); campo `documento_assinatura_id` no contrato; idempotente; (c) hook na CONCLUSÃO do assinador: `data_assinatura` do contrato vira a REAL (última assinatura), o termo assinado (com carimbo/QR de validação) substitui o arquivo do contrato e a publicação no PNCP é disparada automaticamente; (d) cockpit (estação Homologação e contratos), fluxo por contrato fácil de entender: "📝 Gerar termo e colher assinaturas" → "✍️ 1/2 · Fulano ✓ · Beltrano ⏳ (termo PDF · assinar/acompanhar · reenviar)" → "✅ Assinado em dd/mm (termo assinado PDF) · PNCP automático".
- **Degrau 2 COMPLETO** — sem pendências. Contrato nasce, é assinado e ganha eficácia (PNCP) sem sair do sistema.
