...
- **Pendências do Degrau 2:** (2.5) aposentar módulo `contratacao-direta` órfão; (opcional) prorrogação automática da janela de lances; (opcional) resultado PNCP: consulta pública ainda retorna 204 no treina — indicadores obrigatórios enviados um a um pelo PNCP (margem/ME-EPP/desempate incluídos em 25/07, aguardando validação pós-deploy).
