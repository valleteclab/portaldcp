...
import { Loader2, Fuel, ClipboardList, Home, User, ChevronRight, CheckCircle, Clock, XCircle, AlertCircle, LogOut, Lock } from 'lucide-react'
import { API_URL } from '@/lib/api'

// ─── Tipos ────────────────────────────────────────────────────────────────────

interface CredencialInfo {
  id: string; nome: string; cargo?: string; orgao_nome?: string
...
interface VereadorData {
  ...
  mes: string; cota_mensal: number; litros_usados: number
...
const LS_CRED = 'frota_vereador_cred'
...
const getQrUrl = (token: string) => {
  ...
  const url = `${base}/frota/req/${token}`
...
export default function VereadorPage() {
  ...
  // Login state
  const [loginForm, setLoginForm] = useState({ orgaoId: '', codigoAcesso: '', senha: '' })
  const [loginError, setLoginError] = useState('')
  const [loginLoading, setLoginLoading] = useState(false)

  // Novo pedido
  const [veiculos, setVeiculos] = useState<Array<{ id: string; placa: string; modelo: string; marca: string; tipo_combustivel: string }>>([])
  const [formPedido, setFormPedido] = useState({
    veiculo_id: '', veiculo_placa: '', tipo_combustivel: 'GASOLINA',
    quantidade_autorizada: '30', finalidade: '', km_hodometro: '', observacoes: '',
  })
  ...
  const carregarDados = useCallback(async (t: string) => {
    ...
    try {
      ...
      if (!res.ok) { localStorage.removeItem(LS_KEY); setToken(null); setLoading(false); return }
      const d: VereadorData = await res.json()
      setData(d)
      // Carregar veículos vinculados (se houver IDs)
      if (d.credencial.veiculo_ids?.length) {
        // Buscar via frota-pub seria ideal, mas por enquanto usamos a lista vinculada
        // Os veículos serão informados manualmente no pedido
      }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${FROTA_PUB}/auth`, {
        method: 'POST',
      ...
      if (json.credencial?.tipo !== 'VEREADOR') { setLoginError('Este acesso é exclusivo para vereadores'); return }
      localStorage.setItem(LS_KEY, json.token)
      localStorage.setItem(LS_CRED, JSON.stringify(json.credencial))
      setToken(json.token)
  ...
  const handleLogout = () => {
    localStorage.removeItem(LS_KEY); localStorage.removeItem(LS_CRED)
  ...
  const handleEnviarPedido = async () => {
    ...
    try {
      ...
      setFormPedido({ veiculo_id: '', veiculo_placa: '', tipo_combustivel: 'GASOLINA', quantidade_autorizada: '30', finalidade: '', km_hodometro: '', observacoes: '' })
      ...
      <div className="w-full max-w-sm space-y-6">
        ...
        <form onSubmit={handleLogin} className="bg-slate-800 rounded-2xl p-6 space-y-4">
          ...
          <div>
            ...
            <input
              className="w-full bg-slate-700 text-white border border-slate-600 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-orange-400 uppercase"
              placeholder="VER001"
              value={loginForm.codigoAcesso}
              onChange={e => setLoginForm({ ...loginForm, codigoAcesso: e.target.value.toUpperCase() })}
              required
            />
          </div>
          <div>
            ...
            <div className="bg-green-600 rounded-2xl p-4 space-y-3">
              <div className="flex items-center gap-2 text-white">
                ...
                <div>
                  ...
                  <p className="text-green-200 text-sm">Mostre este código no posto</p>
                </div>
              </div>
              ...
              {autorizacao_ativa.token_acesso && (
                <div className="bg-white rounded-xl p-3 flex flex-col items-center gap-2">
                  ...
                  <img
                    src={getQrUrl(autorizacao_ativa.token_acesso)}
                    alt="QR Code da autorização"
                    className="w-[180px] h-[180px]"
                  />
                  <p className="text-slate-500 text-xs text-center">Apresente este QR Code ou o código numérico ao atendente do posto</p>
                </div>
              )}
              ...
              >
                <ClipboardList className="w-4 h-4" /> Mostrar para o Posto
              </button>
            </div>
          </div>
        )}
  ...
  const TabCombustivel = () => (
    <div className="pb-24 px-4 pt-4 space-y-4">
      ...
      {pedidoSucesso ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            >
              {COMB_OPTIONS.map(c => <option key={c} value={c}>{c.charAt(0) + c.slice(1).toLowerCase().replace('nol', 'nol').replace('soli', 'solina').replace('sel', 'sel')}</option>)}
            </select>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button
                onClick={() => setQtdSelecionada('outro')}
                className={`px-4 py-2 rounded-xl border text-sm font-medium transition-colors
            ...
            {qtdSelecionada === 'outro' && (
              <input type="number" step="0.001" min="1"
                ...
                placeholder="Ex: 25"
                value={qtdOutro}
                onChange={e => setQtdOutro(e.target.value)} />
            )}
          </div>
      ...
      {requisicoes.length > 0 && (
        <div>
          <p className="text-xs text-slate-500 font-semibold uppercase tracking-widest mb-2">HISTÓRICO DE PEDIDOS</p>
          <div className="space-y-2">
            {requisicoes.slice(0, 5).map(r => {
              ...
              return (
                <div key={r.id} className="bg-white rounded-2xl p-3 flex items-center gap-3 shadow-sm">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-slate-800 font-mono">{r.codigo}</p>
                    <p className="text-xs text-slate-400">{r.quantidade_autorizada} L · {r.veiculo_placa} · {fmtData(r.data_requisicao)}</p>
                    <p className="text-xs text-slate-400 truncate">{r.finalidade}</p>
                  </div>
                  <span className={`text-xs px-2 py-0.5 rounded-full border whitespace-nowrap ${s.color}`}>{s.label}</span>
                </div>
              )
            })}
          </div>
          ...
          <div key={r.id} className="bg-white rounded-2xl p-4 shadow-sm space-y-2">
            ...
            {r.status === 'AUTORIZADO' && r.token_acesso && (
              <button
                ...
                <CheckCircle className="w-4 h-4" /> Ver QR Code
              </button>
            )}
          </div>
        )
