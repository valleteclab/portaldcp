...
- **Pendências do Degrau 2:** (2.2) PNCP — aviso de contratação direta ao publicar + resultado na homologação (dicionário já mapeado; falta disparo automático); (2.5) aposentar módulo `contratacao-direta` órfão; (opcional) prorrogação automática da janela de lances a cada novo lance no fim (hoje a janela é fixa, como na IN 67).
