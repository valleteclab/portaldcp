...
/**
 * ============================================================================
 * MOTOR DE DISPUTA (disputa) — ÚNICO caminho de lance do pregão/concorrência
 * ============================================================================
 *
...
export interface LancePainelCancelamento {
  id: string;
  valor: number;
  criadoEm: string;
  cancelado: boolean;
...
export interface SolicitacaoCancelamentoPendente {
  lanceId: string;
  itemId: string;
  itemNumero: number;
  fornecedorId: string;
...
export class DisputaService {
  ...
  async getItensPorStatus(sessaoId: string, fornecedorId?: string, opts: OpcoesVisaoDisputa = {}): Promise<{
    ...
    const ativos: Array<{ item_id: string; fornecedor_id: string | null; fornecedor_nome: string | null; valor: string; created_at: Date }> =
      await m.query(
        // Lance final fechado: sigiloso até o fim do prazo (IN 73 art. 24 §2º) — fora das leituras
        `SELECT l.item_id, l.fornecedor_id, l.fornecedor_nome, l.valor, l.created_at
  ...
  private async mapearItemParaDisputa(
    ...
    dados: {
      ...
      lances: Array<{ fornecedor_id: string | null; fornecedor_nome: string | null; valor: string }>;
      totalPropostas: number;
  ...
  async iniciarDisputa(sessaoId: string, itensIds: string[], ator: AtorTransicao): Promise<{ itensIniciados: number; lotesIniciados?: number }> {
    ...
    await this.transicoes.executar(sessao.licitacao_id, AtoLicitacao.INICIAR_DISPUTA, {
      ...
      registro: { origem: 'disputa', sessao_id: sessaoId },
    });
  ...
  async getTodosLances(itemId: string, sessaoId?: string, opts: OpcoesVisaoDisputa = {}): Promise<LanceRegistrado[]> {
    ...
    const saida: LanceRegistrado[] = await Promise.all(
      lances.map(async (l) => {
        let fornecedorId = l.fornecedor_id || ''; // E9: fornecedor_identificador (legado) não é mais lido
  ...
  async listarLancesFornecedorParaCancelamento(sessaoId: string, itemId: string, fornecedorId: string): Promise<LancePainelCancelamento[]> {
  ...
  async listarSolicitacoesCancelamentoPendentes(sessaoId: string): Promise<SolicitacaoCancelamentoPendente[]> {
    const sessao = await this.sessaoRepo.findOne({ where: { id: sessaoId } });
    if (!sessao) throw new NotFoundException('Sessão não encontrada');
  ...
  async cancelarLanceFornecedorImediato(sessaoId: string, itemId: string, lanceId: string, fornecedorId: string): Promise<{ ok: true }> {
    if (await this.lotes.ehLote(itemId)) {
      const p = await this.parametros.daSessao(sessaoId);
  ...
  async solicitarCancelamentoLance(
    sessaoId: string,
    itemId: string,
    lanceId: string,
    fornecedorId: string,
  ...
  async pregoeiroCancelarLance(sessaoId: string, itemId: string, lanceId: string, orgaoId: string, justificativa: string): Promise<{ ok: true }> {
