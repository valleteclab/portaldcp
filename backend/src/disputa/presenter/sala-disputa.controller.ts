...
import { SalaDisputaService } from './sala-disputa.service';

/**
 * Sala de disputa (presenter sobre o motor único — REST /api/disputa).
 *
 * AUTORIZAÇÃO (E1a):
...
export class SalaDisputaController {
  constructor(
    private readonly salaService: SalaDisputaService,
  ...
  async getContextoSessao(@Param('sessaoId') sessaoId: string, @AtorAtual() ator: Ator) {
    ...
    return this.salaService.getContextoSessao(sessaoId);
  }
  ...
  async getContextoPorLicitacao(@Param('licitacaoId') licitacaoId: string, @AtorAtual() ator: Ator) {
    ...
    return this.salaService.getContextoPorLicitacao(licitacaoId);
  }
  ...
  async getBoard(
    ...
    if (ehFornecedor(ator)) {
      ...
      const board = await this.salaService.getBoardFornecedor(sessaoId, fid);
    ...
    return this.salaService.getBoardPregoeiro(sessaoId);
  }
  ...
  async getLancesMeus(
    ...
    return this.salaService.listarLancesMeusParaCancelamento(sessaoId, itemId, fid);
  }
  ...
  async cancelarFornecedor(
    ...
    await this.assertParticipaDaSessao(ator, sessaoId);
    return this.salaService.cancelarLanceFornecedorImediato(sessaoId, itemId, lanceId, fid);
  }
  ...
  async solicitarCancelamento(
    ...
    const fid = this.acesso.fornecedorDoToken(ator);
    await this.assertParticipaDaSessao(ator, sessaoId);
    return this.salaService.solicitarCancelamentoLance(sessaoId, itemId, lanceId, fid, body?.motivo);
  }
  ...
  async pregoeiroCancelar(
    ...
    return this.salaService.pregoeiroCancelarLance(
      sessaoId,
      itemId,
      lanceId,
      dono.orgaoId,
