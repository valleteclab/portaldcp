 ...
 * Namespace `/disputa` (ex-`/disputa-v2`, renomeado na E8; REST `/api/disputa` desde a E9).
 *
 * AUTORIZAÇÃO (E1a):
    ...
    try {
      ...
      if (relacao === 'FORNECEDOR_PARTICIPANTE' && ehFornecedor(ator)) {
        ...
        if (data?.usuarioId && data.usuarioId !== ator.fornecedorId) {
          this.logger.warn(`ACESSO NEGADO: token de ${ator.fornecedorId} declarou usuarioId ${data.usuarioId}`);
      ...
      this.logger.debug(`${tipo} entrou na sessão ${sessaoId}`);
