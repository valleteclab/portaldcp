...
export default function PropostasPage() {
  ...
  const classificarProposta = async (id: string) => {
    ...
    try {
      ...
      toast.error('Erro ao classificar proposta')
    } finally {
  ...
  const desclassificarProposta = async (id: string) => {
    if (!motivoDesclassificacao.trim()) {
      toast.warning('Informe o motivo da desclassificação')
      return
    }
    ...
    try {
      ...
      toast.error('Erro ao desclassificar proposta')
    } finally {
      setProcessando(false)
    }
  }
  ...
  const concluirAnaliseEIniciarDisputa = async () => {
    ...
    if (classificadas.length < 2) {
      toast.warning('É necessário ter pelo menos 2 propostas classificadas para iniciar a disputa')
      return
    }
    
    if (!(await confirmarAcao({ titulo: 'Concluir a análise de propostas?', mensagem: 'Confirma a conclusão da análise de propostas e início da fase de disputa?' }))) {
      return
    }
    ...
    try {
      ...
      if (res.ok) {
        toast.success('Análise concluída! A licitação avançou para a fase de disputa.')
